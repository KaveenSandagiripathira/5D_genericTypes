...
public class FibPair {
    ...
    private static Pair<Integer> nextPairAfter(Pair<Integer> previousPair) {
		Pair<Integer> nextPair = new Pair<Integer>(previousPair.getFirst() + previousPair.getSecond(),previousPair.getSecond());
        return nextPair;
    }
    ...
    private static Integer fib( int n) {
        if (n == 0)
			...
			return nextPairAfter(new Pair<Integer>(fib(n-1),fib(n-2))).getFirst();
		}
    }
}
